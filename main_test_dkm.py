...
from GIMdkm_match import DKM_Matcher
...
def second_match_ensemble(mkpts1, mkpts2, idxs, features_data, key1, key2, lg_matcher, startidx=4096):
    """二次匹配函数，增加索引映射功能确保结果与原始特征点对应
        ...
        for batch_idx in tqdm(range(num_batches)):
            ...
            for i, (idx1, idx2, key1, key2, fname1, fname2) in enumerate(batch_info):
                ...
                if len(idxs) < 700:
                    ...
                    mkpts2 = features_data[key2]['kp'][idxs[:,1]]
                    # 进行第二阶段匹配
                    region_idxs = second_match_ensemble(mkpts1.cpu().numpy(), mkpts2.cpu().numpy(), idxs, features_data, key1, key2, lg_matcher, startidx=match_limit*3)
                    print(f'{key1}-{key2}')
                    print("region_dists:", len(idxs), len(region_idxs))
                    idxs = region_idxs
                n_matches = len(idxs)
    ...
    with h5py.File(f'{feature_dir}/keypoints.h5', mode='w') as f_kp,\
        ...
        for pair_idx in tqdm(index_pairs):
            ...
            with torch.inference_mode():
                ...
                idxs = []
                try:
                    kp1, kp2, dists, idxs = dkm_matcher.match(fname1, fname2)
                except Exception as e:
                    print(f"Error matching {fname1} and {fname2}: {e}")
                    continue
            if len(idxs) == 0:
                continue
            n_matches = len(idxs)
            ...
            idxs = indexer.process_match(key1, key2, kp1.cpu().numpy(), kp2.cpu().numpy())
            ...
            if n_matches >= min_matches:
                ...
                if visualize:
                    ...
                    visualize_matches(fname1, fname2, 
                                   kp1.cpu().numpy(), 
                                   kp2.cpu().numpy(),
                                   np.stack((np.arange(0,len(kp1)),np.arange(0,len(kp1))),axis=1),
                                   save_path)
...
# device=torch.device('cpu')
dkm_matcher = DKM_Matcher(device=device)
