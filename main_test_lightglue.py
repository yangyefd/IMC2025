...
def detect_aliked_second(img_fnames, feature_dir='.featureout', num_features=4096, 
                 resize_to=1024, device=torch.device('cpu')):
    dtype = torch.float32
    extractor = ALIKED(max_num_keypoints=num_features, detection_threshold=0.3, 
                     resize=resize_to).eval().to(device, dtype)
    ...
    with h5py.File(f'{feature_dir}/keypoints.h5', mode='w') as f_kp, \
        ...
        for img_path in tqdm(img_fnames):
            ...
            with torch.inference_mode():
                ...
                feats0 = extractor.extract(image0)
                kpts_alike = feats0['keypoints'].reshape(-1, 2).detach().cpu().numpy()
                kpts[:len(kpts_alike)] = kpts_alike
                descs[:len(kpts_alike),:128] = feats0['descriptors'].reshape(len(kpts_alike), -1).detach().cpu().numpy()
                f_kp[key] = kpts
                f_desc[key] = descs
                f_size[key] = [image0.shape[2:][::-1]]
                f_mask[key] = [len(kpts_alike)]
    return
        ...
        for img_path in tqdm(img_fnames):
            ...
            with torch.inference_mode():
                ...
                # if 1:
                    ...
                    f_kp[key] = kpts
                    f_desc[key] = descs
                    f_size[key] = data['size0'].cpu()*data['scale0'].cpu()
                    f_scale[key] = data['scale0'].cpu()
    ...
    with h5py.File(f'{feature_dir}/keypoints.h5', mode='w') as f_kp, \
        ...
        for img_path in tqdm(img_fnames):
            ...
            with torch.inference_mode():
                try:
                # if 1:
                    kpts = np.zeros((num_features*3,2)).astype(np.float32)
                    descs = np.zeros((num_features*3,256)).astype(np.float32)
                    feats0_lst, data_lst = lightglue_matcher.extract_mr(img_path,nms_radius=2,force=True)
                    for i, feats0 in enumerate(feats0_lst):
                        feats0_kpts = feats0['keypoints0'].reshape(-1, 2).detach().cpu().numpy()
                        kpts[i*num_features:i*num_features+len(feats0_kpts)] = feats0['keypoints0'].reshape(-1, 2).detach().cpu().numpy()
                        descs[i*num_features:i*num_features+len(feats0_kpts)] = feats0['descriptors0'].reshape(len(feats0_kpts), -1).detach().cpu().numpy()
                    ...
                    feats0_alike_pkts_score = feats0_alike['keypoint_scores'].reshape(-1).detach().cpu().numpy()
                    sort_idx = np.argsort(feats0_alike_pkts_score)[::-1]
                    kpts[num_features*2:num_features*2+len(feats0_alike_pkts)] = feats0_alike_pkts[sort_idx]
                    descs[num_features*2:num_features*2+len(feats0_alike_pkts),:128] = feats0_alike_descs[sort_idx]
                    descs[num_features*2:num_features*2+len(feats0_alike_pkts),128:] = feats0_alike_descs[sort_idx]
                    
                    f_kp[key] = kpts
                    f_desc[key] = descs
                    f_size[key] = data_lst[0]['size0'].cpu()*data_lst[0]['scale0'].cpu()
                    f_scale[key] = [data_lst[0]['scale0'].cpu(),data_lst[1]['scale0'].cpu()]
                    f_mask[key] = np.array([len(feats0_kpts), len(feats0_alike_pkts)])
                except Exception as e:
                    print(f"Error processing image {img_fname}: {e}") 
                    # 处理异常情况，例如记录错误或跳过该图像
...
def detect_sp_ensemble_fast(lightglue_matcher, img_fnames, feature_dir='.featureout', num_features=4096, 
                 resize_to=1024, device=torch.device('cpu')):
    #集成方法 ALIke sp各提一半点 2048个
    dtype = torch.float32

    extractor_alike = ALIKED(max_num_keypoints=num_features, detection_threshold=0.3, 
                    resize=resize_to).eval().to(device, dtype)
        ...
        for img_path in tqdm(img_fnames):
            ...
            try:
                with torch.inference_mode():
                    ...
                    kpts_refine = np.zeros((num_features*2,2)).astype(np.float32)
                    descs = np.zeros((num_features*2,256)).astype(np.float32)
                    feats0, data = lightglue_matcher.extract(img_path,nms_radius=2)
                    feats0_kpts = feats0['keypoints0'].reshape(-1, 2).detach().cpu().numpy()
                    kpts[:len(feats0_kpts)] = feats0['keypoints0'].reshape(-1, 2).detach().cpu().numpy()
                    kpts_refine[:len(feats0_kpts)] = feats0['keypoints_refine0'].reshape(-1, 2).detach().cpu().numpy()
                    descs[:len(feats0_kpts)] = feats0['descriptors0'].reshape(len(feats0_kpts), -1).detach().cpu().numpy()
...
def detect_sp_ensemble_fast_mr(lightglue_matcher, img_fnames, feature_dir='.featureout', num_features=4096, 
                 resize_to=1024, device=torch.device('cpu')):
    #集成方法 ALIke sp各提一半点 2048个
    dtype = torch.float32
    ...
    with h5py.File(f'{feature_dir}/keypoints.h5', mode='w') as f_kp, \
        ...
        for img_path in tqdm(img_fnames):
            ...
            if 1:
                with torch.inference_mode():
                    kpts = np.zeros((num_features*f_len,2)).astype(np.float32)
                    kpts_refine = np.zeros((num_features*f_len,2)).astype(np.float32)
                    descs = np.zeros((num_features*f_len,256)).astype(np.float32)
                    feats0_lst, data_lst = lightglue_matcher.extract_mr(img_path,nms_radius=2,mr_lst=mr_lst)
                    mask_lst = []
                    for i, (feats0, data) in enumerate(zip(feats0_lst, data_lst)):
                        feats0_kpts = feats0['keypoints0'].reshape(-1, 2).detach().cpu().numpy()
                        kpts[num_features*i:num_features*i+len(feats0_kpts)] = feats0['keypoints0'].reshape(-1, 2).detach().cpu().numpy()
                        kpts_refine[num_features*i:num_features*i+len(feats0_kpts)] = feats0['keypoints_refine0'].reshape(-1, 2).detach().cpu().numpy()
                        descs[num_features*i:num_features*i+len(feats0_kpts)] = feats0['descriptors0'].reshape(len(feats0_kpts), -1).detach().cpu().numpy()
                        mask_lst.append(len(feats0_kpts))
                    image0 = load_torch_image(img_path, device=device).to(dtype)
                    feats0_alike = extractor_alike.extract(image0)
                    feats0_alike_pkts = feats0_alike['keypoints'].reshape(-1, 2).detach().cpu().numpy()
                    kpts[num_features*(i+1):num_features*(i+1)+len(feats0_alike_pkts)] = feats0_alike_pkts
                    descs[num_features*(i+1):num_features*(i+1)+len(feats0_alike_pkts),:128] = feats0_alike['descriptors'].reshape(len(feats0_alike_pkts), -1).detach().cpu().numpy()
                    descs[num_features*(i+1):num_features*(i+1)+len(feats0_alike_pkts),128:] = feats0_alike['descriptors'].reshape(len(feats0_alike_pkts), -1).detach().cpu().numpy()
                    mask_lst.append(len(feats0_alike_pkts))

                    f_kp[key] = kpts
                    f_desc[key] = descs
                    f_size[key] = [data['size0'].cpu()[0]*data['scale0'].cpu()[0]]
                    f_scale[key] = [data['scale0'].cpu() for data in data_lst]
                    f_mask[key] = np.array(mask_lst)
...
# def multi_resolution_match(mkpts1, mkpts2, idxs, match_scores, features_data, key1, key2, lg_matcher, num_features, startidx=4096):
#     """
#     根据现有匹配对，判断两张图的缩放关系，然后选取对应的索引其中
#     features_data[key1]['kp'][:num_features]: 原分辨率
#     features_data[key2]['kp'][num_features:num_features*2]: 原分辨率
#     features_data[key1]['kp'][num_features:]: 缩放后的分辨率
#     Args:
#         mkpts1, mkpts2: 初次匹配的特征点对
#         idxs: 初次匹配的索引对
...
def match_with_gimlightglue_ensemble_withfine_mr(lightglue_matcher, img_fnames, index_pairs, feature_dir='.featureout', 
                                           device=torch.device('cpu'), min_matches=20, batch_size=2, 
                                           tok_limit=1200, match_limit=4096, verbose=True, visualize=True):
    """
    使用批处理方式进行特征匹配，点数不会超过 max_points，但可能小于。
        ...
        for batch_idx in tqdm(range(num_batches)):
            ...
            for pair_idx in batch_pairs:
                ...
                pred = {
                    ...
                    'size0': features_data[key1]['size'],
                    'size1': features_data[key2]['size'],
                    'scale0': features_data[key1]['scale'][0],
                    'scale1': features_data[key2]['scale'][0],
                }

                pred_mr = {
                    'keypoints0': kp1[match_limit:match_limit+1024][None],
                    'keypoints1': kp2[match_limit:match_limit+1024][None],
                    'descriptors0': desc1[match_limit:match_limit+1024][None],
                    'descriptors1': desc2[match_limit:match_limit+1024][None],
                    'size0': features_data[key1]['size'],
                    'size1': features_data[key2]['size'],
                    'scale0': features_data[key1]['scale'][1],
                    'scale1': features_data[key2]['scale'][1],
                }

                batch_data.append(pred)
                batch_data_mr.append(pred_mr)
                batch_info.append((idx1, idx2, key1, key2, fname1, fname2))
            ...
            batch_preds_mr = {
                'keypoints0': torch.cat([data['keypoints0'] for data in batch_data_mr], dim=0).to(device),
                'keypoints1': torch.cat([data['keypoints1'] for data in batch_data_mr], dim=0).to(device),
                'descriptors0': torch.cat([data['descriptors0'] for data in batch_data_mr], dim=0).to(device),
                'descriptors1': torch.cat([data['descriptors1'] for data in batch_data_mr], dim=0).to(device),
            ...
            for i, (idx1, idx2, key1, key2, fname1, fname2) in enumerate(batch_info):
                    ...
                    mask2_alike = features_data[key2]['mask'][-1]
                    pred_alike = {
                        'keypoints0': kp1[4096*2:][:mask1_alike][None].to(device),
                        'keypoints1': kp2[4096*2:][:mask2_alike][None].to(device),
                        'descriptors0': desc1[4096*2:,:128][:mask1_alike].to(device),
                        'descriptors1': desc2[4096*2:,:128][:mask2_alike].to(device),
                    }
            ...
            sorted_dists = []
            for i in range(len(batch_dists)):
                if len(batch_dists[i]) > 0 or len(batch_dists_fine[i]) > 0 or len(batch_dists_mr[i]) > 0:
                    batch_idxs_mr_t = batch_idxs_mr[i] + 4096
                    dists = torch.cat([batch_dists[i],batch_dists_mr[i],batch_dists_fine[i]])
                    idxs = torch.cat([batch_idxs[i],batch_idxs_mr_t,batch_idxs_fine[i]])
            ...
            for i, (idx1, idx2, key1, key2, fname1, fname2) in enumerate(batch_info):
                ...
                try:
                    if len(idxs) < 800:
                        ...
                        mkpts2 = features_data[key2]['kp'][idxs[:,1]]
                        # 进行第二阶段匹配
                        region_idxs, region_dists = second_match_ensemble(mkpts1.cpu().numpy(), mkpts2.cpu().numpy(), idxs, match_scores, features_data, key1, key2, lg_matcher, device, startidx=2*4096)
                        print(f'{key1}-{key2}')
                        print("region_dists:", len(idxs), len(region_idxs))
                        idxs = region_idxs[:1500]
                        match_scores = region_dists[:1500]
        ...
        for pair_idx in index_pairs:
            ...
            # if 1:
                ...
                pred = {
                    ...
                    'keypoints1': kp2[:mask2][None],
                    'descriptors0': desc1[:mask1][None],
                    'descriptors1': desc2[:mask2][None],
                    'size0': features_data[key1]['size'],
                    'size1': features_data[key2]['size'],
                    'scale0': features_data[key1]['scale'],
                    'scale1': features_data[key2]['scale'],
                ...
                # 处理结果
                run_pairs += 1
                if len(batch_idxs[0]) == 0:
                    continue
                
                idxs = batch_idxs[0]
                match_scores = batch_dists[0]
...
def match_with_gimlightglue_ensemble_fast_mr(lightglue_matcher, img_fnames, index_pairs, feature_dir='.featureout', 
                                           device=torch.device('cpu'), min_matches=20, 
                                           tok_limit=1200, match_limit=4096, verbose=True, visualize=True):
    """
    使用批处理方式进行特征匹配，点数不会超过 max_points，但可能小于。
    ...
    with h5py.File(f'{feature_dir}/matches.h5', mode='w') as f_match:
        ...
        for pair_idx in index_pairs:
            ...
            if 1:
                ...
                match_count = 0
                for i in range(2):
                    kp1 = features_data[key1]['kp'][i*4096:(i+1)*4096]
                    kp2 = features_data[key2]['kp'][i*4096:(i+1)*4096]
                    desc1 = features_data[key1]['desc'][i*4096:(i+1)*4096]
                    desc2 = features_data[key2]['desc'][i*4096:(i+1)*4096]
                    mask1 = features_data[key1]['mask'][i]
                    mask2 = features_data[key2]['mask'][i]
                    ...
                    pred = {
                        ...
                        'size0': features_data[key1]['size'],
                        'size1': features_data[key2]['size'],
                        'scale0': features_data[key1]['scale'][i],
                        'scale1': features_data[key2]['scale'][i],
                    }
                ...
                # 合并两个匹配结果
                dists = torch.cat(dists_lst)
                idxs = torch.cat(idxs_lst)
                batch_info = (idx1, idx2, key1, key2, fname1, fname2)
                dists, idxs = match_nms(dists, idxs, batch_info, features_data, 1)
                dists = dists[:tok_limit]
                idxs = idxs[:tok_limit]
                # 处理结果
                run_pairs += 1
                if len(idxs) == 0:
...
def match_with_lightglue_second(img_fnames, index_pairs, feature_dir='.featureout', 
                                           device=torch.device('cpu'), min_matches=20, batch_size=2, 
                                           tok_limit=1200, match_limit=4096, verbose=True, visualize=True):
    """
    使用批处理方式进行特征匹配，点数不会超过 max_points，但可能小于。
    ...
    with h5py.File(f'{feature_dir}/keypoints.h5', mode='r') as f_kp, \
        ...
        for img_path in tqdm(img_fnames):
            key = img_path.split('/')[-1].split('\\')[-1]
            features_data[key] = {
                'kp': torch.from_numpy(f_kp[key][...]).float().to(device),
                'desc': torch.from_numpy(f_desc[key][...]).float().to(device),
                'size': torch.from_numpy(f_size[key][...]).float().to(device),
                'mask': torch.from_numpy(f_mask[key][...]).to(device)
            }
    ...
    with h5py.File(f'{feature_dir}/matches.h5', mode='w') as f_match:
        for pair_idx in tqdm(index_pairs):
            ...
            mask1 = features_data[key1]['mask'][-1]
            mask2 = features_data[key2]['mask'][-1]
            kp1 = features_data[key1]['kp'][:mask1]
            kp2 = features_data[key2]['kp'][:mask2]
            desc1 = features_data[key1]['desc'][:mask1,:128]
            desc2 = features_data[key2]['desc'][:mask2,:128]
            ...
            if len(idxs) < 800:
                ...
                mkpts2 = features_data[key2]['kp'][idxs[:,1]]
                # 进行第二阶段匹配
                region_idxs, region_dists = second_match_ensemble(mkpts1.cpu().numpy(), mkpts2.cpu().numpy(), idxs, match_scores, features_data, key1, key2, lg_matcher, startidx=0)
                print(f'{key1}-{key2}')
                print("region_dists:", len(idxs), len(region_idxs))
                idxs = region_idxs
                match_scores = region_dists
...
for dataset, predictions in samples.items():
    ...
    if 1:
        ...
        match_matrix = match_with_gimlightglue_ensemble_withfine_mr(lightglue_matcher, images, index_pairs, feature_dir=feature_dir, device=device, verbose=False)
