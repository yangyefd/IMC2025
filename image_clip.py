...
def process_folder(image_folder):
    ...
    # 保存结果到CSV文件
    datasets_name = image_folder.split('/')[-1]
    results_dir = os.path.join("similarity_results",datasets_name)
...
if __name__ == "__main__":
    ...
    # 图像文件夹路径
    image_folder = "../image-matching-challenge-2025/train"

    for dataset in os.listdir(image_folder):
        image_folder_one = os.path.join(image_folder, dataset)
        if os.path.isdir(image_folder_one):
            process_folder(image_folder_one)
        else:
            print(f"跳过非文件夹: {image_folder_one}")    
