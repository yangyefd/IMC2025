...
class LoFTR(nn.Module):
    ...
    def forward_match(self, data):
        ...
        feat_f0 = data['feat_f0']
        feat_f1 = data['feat_f1']
    ...
    def forward_coarse(self, data):
        ...
        Args:
            data (dict): 包含以下键值:
                'feat_c0': (torch.Tensor): (N, C, H, W) 第一张图的粗特征
                'feat_c1': (torch.Tensor): (N, C, H, W) 第二张图的粗特征
    ...
    def forward_fine(self, data):
        """细匹配forward pass
        Args:
            data (dict): 包含粗匹配结果的字典
            feat_f0 (torch.Tensor): (N, C, H, W) 第一张图的细特征
            feat_f1 (torch.Tensor): (N, C, H, W) 第二张图的细特征
        """
        # 1. 细特征预处理
        feat_f0_unfold, feat_f1_unfold = self.fine_preprocess.forward_kp(data['feat_f0'], data['feat_f1'], 
                                                             data['mkpts0_c'], data['mkpts1_c'], 
                                                             data)

        # 2. 细匹配transformer处理
        if feat_f0_unfold.size(0) != 0:  # 至少有一个粗匹配点
            feat_f0_unfold, feat_f1_unfold = self.loftr_fine(feat_f0_unfold, feat_f1_unfold)
