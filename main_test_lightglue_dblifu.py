...
for dataset, predictions in samples.items():
    ...
    for pair_id, match_array in matches_data:
        image_id1, image_id2 = pair_id_to_image_pair(pair_id)

        if len(match_array) < 8:
            continue  # 至少需要8个点用于F矩阵估计
        if len(match_array) > max_matches:
            max_matches = len(match_array)
            max_pair = pair_id
        keypoints1 = read_keypoints(database_path, image_id1)
        keypoints2 = read_keypoints(database_path, image_id2)

        if len(match_array) < 8:
            continue

        pts1 = keypoints1[match_array[:, 0], :2]
        pts2 = keypoints2[match_array[:, 1], :2]
        confidence = np.ones(len(pts1))  # 可用NNDR或其他方式生成

        F, inlier_mask = prosac_lo_ransac_fundamental(pts1, pts2, confidence,threshold=0.3)

        if F is not None:
            inlier_matches = match_array[inlier_mask]
           
        # 写入两视图几何
        write_two_view_geometry(database_path, image_id1, image_id2, F, inlier_matches)
